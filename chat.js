window.onload = function() {
  ...
  // Creating a new WebSocket connection.
    ...
    socket.onopen = function(event) { //Introducing a WebSocket opening message
        socketStatus.innerHTML = 'Connected to: ' + event.currentTarget.URL;
        socketStatus.className = 'open';
    };

    socket.onerror = function(error) { //Code referring to occurrence of errors
        console.log('WebSocket error: ' + error);
    };

    socket.send(data); 

    form.onsubmit = function(e) { //Sending the form data
        e.preventDefault();
